...
function getConfig() {
    ...
    return {
        port: config.get('port', 52323),
        httpPort: config.get('httpPort', 52325),
        autoReconnect: config.get('autoReconnect', true),
        reconnectDelay: config.get('reconnectDelay', 2000),
        maxDataPoints: config.get('maxDataPoints', 30),
        commandArgs: config.get('commandArgs', ['collect', '--no-auth', '--urls', `https://localhost:${config.port},http://localhost:${config.httpPort}`])
    };
}
...
async function fetchWithAgent(url) {
    ...
    if (isHttps) {
        ...
        return new Promise((resolve, reject) => {
            https_1.default.get(url, { rejectUnauthorized: false }, (res) => {
                let data = '';
...
async function checkDotnetMonitor() {
    try {
...
async function esperarAPI(url, maxIntentos = 30) {
...
function iniciarStreamMetricas(pid, panel) {
    ...
    const conectar = () => {
        ...
        requestActual = https_1.default.get(url, { rejectUnauthorized: false }, (response) => {
...
function activate(context) {
    ...
    const dashboardCmd = vscode.commands.registerCommand('dotnetMonitor.showDashboard', async () => {
        const config = getConfig();
        // Verificar si dotnet-monitor está instalado
        const monitorCheck = await checkDotnetMonitor();
        if (!monitorCheck.installed) {
            vscode.window.showErrorMessage('dotnet-monitor no está instalado. Ejecuta: dotnet tool install -g dotnet-monitor');
            return;
        }
        const panel = vscode.window.createWebviewPanel('dotnetDashboard', 'Dotnet Monitor Dashboard', vscode.ViewColumn.One, {
            enableScripts: true
        });
        ...
        panel.webview.onDidReceiveMessage(async (mensaje) => {
